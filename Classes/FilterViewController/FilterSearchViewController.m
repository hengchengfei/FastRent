...
#define FilterParams_ID(frItem) type == [TypeSource integerValue] ? @"sourceId":type ==[TypePrice integerValue]?@"priceId":@"rentTypeId"
...
@interface FilterSearchViewController ()
{
    ...
    PDMoreSearchViewController *_pdMoreViewController;
...
- (void)viewDidLoad
{
    ...
    NSArray *arrayText=[NSArray arrayWithObjects:@"来源",@"租金",@"租住类型", nil];
    ...
    NSArray *arrayType =@[TypeSource,TypePrice,TypeRentType,@-1];//此处的类型一定要和对应的类型一致
    //
...
//    
  ...
  [self.view addSubview:frView];
    ...
    //添加筛选下拉列表
    _pdMoreViewController=[[PDMoreSearchViewController alloc]initWithDataSource:self.comboxDatasource];
...
-(void)PDMoreOKClick:(NSNumber *)houseTypeId order:(NSNumber *)orderId
{
